...
#### Scenario: Dify возвращает строку без экспертной оценки закрытия
- **WHEN** Dify result содержит критерий или требование без `coverageStatus` и `coverageAmount`
- **THEN** система SHALL сохранить строку как корректную документную строку блока критериев, оставив экспертные поля пустыми для тендерного специалиста
